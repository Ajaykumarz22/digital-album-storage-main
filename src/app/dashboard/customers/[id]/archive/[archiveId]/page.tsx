...
export default async function StudioArchiveDetailPage({
  params,
}: {
  params: Promise<{ id: string; archiveId: string }>;
}) {
  ...
  if (!mongoose.isValidObjectId(archiveId)) notFound();
    ...
    <div className="mx-auto max-w-4xl space-y-8 px-6 py-12">
      <div>
        <Link href={base} className="text-sm text-blue-600 hover:underline">
          ← Back to {customer.name || customer.email}
        </Link>
      ...
      <RestoreControls
        ...
        availableUntil={
          archive.status === "available"
            ? formatDate(archive.restore?.availableUntil)
            : null
        }
      />
